...
# Use only tensorflow.keras (no standalone keras)
from tensorflow.keras import backend as K
from tensorflow.keras import Input, Model
from tensorflow.keras.layers import (
    Activation,
    Conv3D,
    Conv3DTranspose,   # replaces old Deconvolution3D
    MaxPooling3D,
    UpSampling3D,
    concatenate,
)
...
def visualize_data_gif(data_):
    ...
    # Save GIF in a cross-platform temporary directory
    gif_path = os.path.join(tempfile.gettempdir(), "gif.gif")
...
def create_convolution_block(input_layer, n_filters, batch_normalization=False,
    ...
    """
    Basic 3D convolutional block with optional activation.
    """
    # Ensure n_filters is an integer
...
def get_up_convolution(n_filters, pool_size, kernel_size=(2, 2, 2),
                       strides=(2, 2, 2), use_transpose=False):
    """
    Returns either Conv3DTranspose (deconvolution) or UpSampling3D.
    """
    # Ensure n_filters is an integer
    n_filters = int(n_filters)
    if use_transpose:
...
def unet_model_3d(loss_function, input_shape=(4, 160, 160, 16),
                  ...
                  use_transpose=False, depth=4, n_base_filters=32,
                  metrics=[], batch_normalization=False,
    ...
    levels = list()
    filter_counts = []  # Keep track of filter counts to avoid shape access issues

    # Downsampling path
    for layer_depth in range(depth):
        n_filters_1 = n_base_filters * (2 ** layer_depth)
        n_filters_2 = n_base_filters * (2 ** layer_depth) * 2
        ...
        filter_counts.append(n_filters_2)  # Store the filter count
    ...
    for layer_depth in range(depth - 2, -1, -1):
        # Use stored filter count instead of accessing layer shape
        current_filters = filter_counts[layer_depth + 1] if layer_depth + 1 < len(filter_counts) else filter_counts[-1]
        
        up_convolution = get_up_convolution(pool_size=pool_size,
                                            use_transpose=use_transpose,
                                            n_filters=current_filters)(current_layer)
        concat = concatenate([up_convolution, levels[layer_depth][1]], axis=1)
        
        # Use stored filter count for this level
        level_filters = filter_counts[layer_depth]
